...
namespace Vege.WeChatOauth
{
    public class WeChatHandler : OAuthHandler<WeChatOptions>
    {
        ...
        protected async override Task<AuthenticateResult> HandleRemoteAuthenticateAsync()
        {
            ...
            {
                return AuthenticateResult.Fail("Corelation failed");
            }
        ...
        protected override async Task<AuthenticationTicket> CreateTicketAsync(ClaimsIdentity identity, AuthenticationProperties properties, OAuthTokenResponse tokens)
        {
            var queryBuilder = new QueryBuilder()
            {
                ...
                {"openid",tokens.TokenType },
                {"lang","zh_CN" }
            };
            var infoRequest = Options.UserInformationEndpoint + queryBuilder.ToString();

            var res = await Backchannel.GetAsync(infoRequest, Context.RequestAborted);
            ...
            var user = JObject.Parse(await res.Content.ReadAsStringAsync());
            ...
            if (!StringValues.IsNullOrEmpty(identifier))
            {
